...
Expected pre-approval result after the compact pilot plus first cache-fill batch:
`planned_calls=60`, `cached_calls=20`, and `missing_calls=40`.
...
Expected current result: 4 batches, 10 calls per batch, approximately 41k
...
`20/60` cached and remains outside paper claims.
...
Expected current result: `checks=438 failures=0`.
